...
if(user.role == "vendor"){
  ...
  if(snapshots.docs.length >0){
        ...
        $("#user_restaurant_name").append('<label class="col-3 control-label">{{trans('lang.restaurant')}}</label><input type="text" class="col-9 form-control vendor_restaurant" value="'+data.title+'">');
          

  }else{

    $("#user_restaurant_name").append('<label class="col-3 control-label">{{trans('lang.restaurant')}}</label><input type="text" class="col-9 form-control vendor_restaurant" >');
  }
...
//var photo ="https://assets.bonappetit.com/photos/5d03bea59ffc67bff3c6f86e/master/pass/HLY_Lentil_Burger_Horizontal.jpg";
 ...
 var user_name = userFirstName+" "+userLastName;
 	...
 	var vendorRestaurantSelect = $("#vendor_restaurant_select option:selected").val();
 		 ...
 		 if(vendorRestaurantSelect != undefined && role == "vendor"){
      console.log(vendorRestaurantSelect);
       database.collection('users').doc(id).update({'firstName':userFirstName,'lastName':userLastName,'email':email,'phoneNumber':userPhone,'isActive':active,'profilePictureURL':photo,'vendorID':vendorRestaurantSelect,'role':role}).then(function(result) {
                
             });

               database.collection('vendors').doc(vendorRestaurantSelect).update({'author':id,'authorProfilePic':photo,'authorName':user_name}).then(function(result) {
  ...
  $( "#user_role" ).change(function() {
    $("#user_restaurant_set_option").hide();
    var user_role = $("#user_role option:selected").val();

    if(user_role == "vendor"){
      $("#user_restaurant_set_option").show();
